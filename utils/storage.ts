...
export function manageServerInfo(
  wordId: string,
  whatToChange: keyof ServerInfoObject,
  whatToDo: string,
  num?: string
) {
  ...
  if (whatToDo === 'add') {
    if (
      whatToChange === 'learnt' &&
      !Object.keys(serverInfoObject[whatToChange]).includes(wordId)
    ) {
    ...
    if (
      whatToDo === 'raise' &&
      whatToChange === 'howManyInARow' &&
      ((+num >= 3 && !serverInfoObject.difficult[wordId]) || +num >= 5)
    ) {
      serverInfoObject.learnt[wordId] = true;
    } else if (whatToDo === 'lower') {
      if (Object.keys(serverInfoObject.learnt).includes(wordId))
...
export let storage = new Proxy(storageObject, {
  set: function (target: StorageObject, key: keyof StorageObject, value) {
    if (
      //number
      key === 'bookGroup' ||
      key === 'bookPage' ||
...
if (localStorageInit !== null) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    if (
      ...
      storage[key] = value;
    } else if (key === 'isAuthorized' && typeof value === 'boolean') {
      storage[key] = value;
    } else if (
      (key === 'userId' ||
        key === 'token' ||
      ...
      (key === 'markedDifficult' || key === 'markedDeleted') &&
      Array.isArray(value) &&
