...
export function checkChoice(id: string | null) {
  const buttonPressed = document.querySelector(
    `#gameOption-${id}`
  ) as HTMLElement;
  ...
  const wrapper = document.querySelector(
    `#wrapper-${storageT.currentGameMode}`
  );
  if (wrapper) {
    if (id === storageT.rightAnswer.id) {
      if (storageT.currentGameMode === 'sniper')
        storageT.currentBirdStatus = 'dead';
      const inARowData =
        serverInfoObject.howManyInARow[storageT.rightAnswer.id];
      ...
      const totalRightAnswers = totalRightAnswersData
        ? totalRightAnswersData
        : 0;
      manageServerInfo(
        storageT.rightAnswer.id,
        'howManyRight',
      ...
      if (storageT.currentGameMode === 'sniper')
        storageT.currentBirdStatus = 'escaped';
      ...
      const totalWrongAnswers = totalWrongAnswersData
        ? totalWrongAnswersData
        : 0;
      manageServerInfo(
        storageT.rightAnswer.id,
        'howManyWrong',
